...
;_SCN_AddControlButtons	  - Adds the selected control buttons to the gui. (Close,Maximize,Minimize,Fullscreen Toogle, Menu button)
;_SCN_GUIDelete			  - Destroys all created metro buttons,checkboxes,radios etc., deletes the GUI and reduces memory usage.
;_SCN_EnableHighDPIScaling  - Enables high DPI support: Detects the users DPI settings and resizes GUI and all controls to look perfectly sharp.
;_SCN_EnableOnEventMode - Allows using the MetroUDF with OnEventMode enabled
;_SCN_SetGUIOption    	  - Allows to set different options like dragmove, resize and min. resize width/height.
;_SCN_FullscreenToggle 	  - Toggles between fullscreen and normal window. NOTE: $AllowResize has to be set True when creating a gui and this also requires the creation of a fullscreen control button.
;_SCN_AddControlButton_Back - Creates a back button on the left+top side of the gui.
;_SCN_MenuStart 			  	- Shows/creates a menu window that slides in from the right side of the gui. (Similar to Android menus or Windows 10 calculator app)
;_SCN_RightClickMenu		- Shows/creates a rightclick menu window with the provided button names.

;==========================================Buttons=================================================
;_SCN_CreateButton   - Creates metro style buttons. Hovering creates a frame around the buttons.
;_SCN_CreateButtonEx - Creates Windows 10 style buttons with a frame around. Hovering changes the button color to a lighter color.
;_SCN_CreateButtonEx2 - Creates a button with slightly rounded corners and no frame. Hovering changes the button color to a lighter color.
;_SCN_DisableButton  - Disables a metro button and adds a grayed out look to it.
;_SCN_EnableButton   - Enables a metro button and removes grayed out look of it.

;==========================================Toggles=================================================
;_SCN_CreateToggle - Creates a Windows 10 style toggle with a text on the right side.(NEW Style)
;_SCN_CreateToggleEx - Creates a Windows 8 style toggle with a text on the right side.
;_SCN_ToggleIsChecked - Checks if a toggle is checked or not. Returns True or False.
;_SCN_ToggleCheck - Checks/Enables a toggle.
;_SCN_ToggleUnCheck - Unchecks/Disables a toggle.
;_SCN_ToggleSwitch - Toggles between checked/unchecked state and then returns the current state.  -> Should only be used to handle user clicks

;===========================================Radios=================================================
;_SCN_CreateRadio - Creates a metro style radio.
;_SCN_CreateRadioEx - Creates a metro style radio with colored checkmark.
;_SCN_RadioCheck - Checks the selected radio and unchecks all other radios in the selected group.
;_SCN_RadioIsChecked - Checks if the radio in a specific group is selected.

;==========================================Checkboxes==============================================
;_SCN_CreateCheckbox - Creates a modern looking checkbox.
;_SCN_CreateCheckboxEx - Creates a classic-style checkbox with the default black white colors.
;_SCN_CreateCheckboxEx2 - Creates a modern rounded checkbox.
...
;=============================================Progress=============================================
;_SCN_CreateProgress - Creates a simple progressbar.
;_SCN_SetProgress - Sets the progress in % of a progressbar.
...
; Syntax ........: _SCN_CreateGUI($Title, $Width, $Height[, $Left = -1[, $Top = -1[, $AllowResize = False[, $ParentGUI = ""]]]])
...
; Example .......: _SCN_CreateGUI("Example", 500, 300, -1, -1, True)
; ===============================================================================================================================
Func _SCN_CreateGUI($Title, $Width, $Height, $Left = -1, $Top = -1, $AllowResize = False, $ParentGUI = "")
	...
	If $AllowResize Then
		...
		$gID = _SCN_SetGUIOption($GUI_Return, True, True, $Width, $Height)
	...
	Else
		...
		$gID = _SCN_SetGUIOption($GUI_Return, False, False, $Width, $Height)
		DllCall("uxtheme.dll", "none", "SetThemeAppProperties", "int", BitOR(1, 2, 4))
	EndIf
	...
	Return ($GUI_Return)
EndFunc   ;==>_SCN_CreateGUI

; #FUNCTION# ====================================================================================================================
; Name ..........: _SCN_SetGUIOption
; Description ...: Allows to set different options like dragmove, resize and min. resize width/height.
; Syntax ........: _SCN_SetGUIOption($mGUI[, $AllowDragMove = False[, $AllowResize = False[, $Win_MinWidth = ""[,
;                  $Win_MinHeight = ""]]]])
...
; Example .......: _SCN_SetGUIOption($Form1, True, True, 400, 300)
; ===============================================================================================================================
...
; #FUNCTION# ====================================================================================================================
; Name ..........: _SCN_AddControlButtons
; Description ...: Creates the selected control buttons for a metro style gui.
; Syntax ........: _SCN_AddControlButtons([$CloseBtn = True[, $MaximizeBtn = True[, $MinimizeBtn = True[, $FullScreenBtn = True[,
;                  $MenuBtn = False]]]]])
...
;                  $MenuBtn             - [optional] True/False. Default is False.;Adds a Menu Button that can be used with _SCN_MenuStart
...
;				   Array[6] = Menu button
; Example .......: _SCN_AddControlButtons(True, True, True, True, True)
; ===============================================================================================================================
...
EndFunc   ;==>_SCN_AddControlButtons

; #FUNCTION# ====================================================================================================================
; Name ..........: _SCN_EnableHighDPIScaling
...
; Syntax ........: _SCN_EnableHighDPIScaling()
; ===============================================================================================================================
Func _SCN_EnableHighDPIScaling($Enable = True)
	$HIGHDPI_SUPPORT = $Enable
EndFunc   ;==>_SCN_EnableHighDPIScaling

; #FUNCTION# ====================================================================================================================
; Name ..........: _SCN_EnableOnEventMode
; Description ...: Allows using the UDF with OnEventMode enabled.
; Syntax ........: _SCN_EnableOnEventMode([$Enable = True])
; ===============================================================================================================================
Func _SCN_EnableOnEventMode($Enable = True)
	$mOnEventMode = $Enable
EndFunc   ;==>_SCN_EnableOnEventMode

; #FUNCTION# ====================================================================================================================
; Name ..........: _SCN_FullscreenToggle
...
; Syntax ........: _SCN_FullscreenToggle($mGUI, $Control_Buttons_Array)
; Parameters ....: $mGUI                  - Handle to the GUI.
;                  $Control_Buttons_Array - Array containing the control button handles as returned from _SCN_AddControlButtons.
...
; ===============================================================================================================================
Func _SCN_FullscreenToggle($mGUI)
	GUISetState(@SW_SHOW, $mGUI) ;Fixes a bug that occurs when using multiple child windows
	Local $iGui_Count = _iGetGUIID($mGUI)

	If ($iGui_Count == "") Then
		ConsoleWrite("Fullscreen-Toggle failed: GUI not registered. Not created with _SCN_CreateGUI ?" & @CRLF)
		Return SetError(1) ;
	EndIf
	If Not $iGUI_LIST[$iGui_Count][2] Then
		ConsoleWrite("Fullscreen-Toggle failed: GUI is not registered for resizing. Please use _SCN_SetGUIOption to enable resizing." & @CRLF)
		Return SetError(2) ;
	EndIf
	...
	EndIf
EndFunc   ;==>_SCN_FullscreenToggle



; #FUNCTION# ====================================================================================================================
; Name ..........: _SCN_AddControlButton_Back
; Description ...: Creates a back button on the left+top side of the gui.
; Syntax ........: _SCN_AddControlButton_Back([, $GUI_BG_Color = $GUIThemeColor[, $GUI_Font_Color = $FontThemeColor]])
...
; Example .......: _SCN_AddControlButton_Back()
; ===============================================================================================================================
Func _SCN_AddControlButton_Back($GUI_BG_Color = $GUIThemeColor, $GUI_Font_Color = $FontThemeColor, $tMargin = 2)
...
EndFunc   ;==>_SCN_AddControlButton_Back
...
; Syntax ........: _SCN_MenuStart($mGUI, $Metro_MenuBtn, $mWidth, $ButtonsArray)
; Parameters ....: $mGUI                - Handle to the gui.
;                  $Metro_MenuBtn       - Handle to the menu button that is returned by _SCN_AddControlButtons. (this would be $Array[6] returned by _SCN_AddControlButtons function)
...
Func _SCN_RightClickMenu($mGUI, $mWidth, $ButtonsArray, $bFont = "Segoe UI", $bFontSize = 9, $bFontStyle = 0)
...
; Syntax ........: _SCN_DisableButton($mButton)
; Parameters ....: $mButton             - Handle to the button.
; Example .......: _SCN_DisableButton($Button1)
; ===============================================================================================================================
Func _SCN_DisableButton($mButton)
	Local $CurrentGUI = GetCurrentGUI()
	...
	For $i_BTN = 0 To (UBound($iHoverReg) - 1)
		...
		EndIf
	Next
EndFunc   ;==>_SCN_DisableButton

; #FUNCTION# ====================================================================================================================
; Name ..........: _SCN_EnableButton
; Description ...: Enables a metro style button and reverts the grayed out font style.
; Syntax ........: _SCN_EnableButton($mButton)
; Parameters ....: $mButton             - Handle to the button.
; ===============================================================================================================================
Func _SCN_EnableButton($mButton)
	Local $CurrentGUI = GetCurrentGUI()
...
; Syntax ........: _SCN_CreateToggle($Text, $Left, $Top, $Width, $Height[, $BG_Color = $GUIThemeColor[,
...
; ===============================================================================================================================
Func _SCN_CreateToggle($Text, $Left, $Top, $Width, $Height, $BG_Color = $GUIThemeColor, $Font_Color = $FontThemeColor, $Font = "Segoe UI", $Fontsize = "11")
	Local $Text1 = $Text
	If $Height < 20 Then
...
EndFunc   ;==>_SCN_CreateToggle

; #FUNCTION# ====================================================================================================================
; Name ..........: _SCN_CreateToggleEx
...
; Return values .: Handle to the toggle.
; ===============================================================================================================================
Func _SCN_CreateToggleEX($Text, $Left, $Top, $Width, $Height, $BG_Color = $GUIThemeColor, $Font_Color = $FontThemeColor, $Font = "Segoe UI", $Fontsize = "11")
	...
	Return $Toggle_Array[0]
EndFunc   ;==>_SCN_CreateToggleEX

; #FUNCTION# ====================================================================================================================
; Name ..........: _SCN_CreateToggle
...
; Return values .: Handle to the toggle.
; ===============================================================================================================================
Func _SCN_CreateOnOffToggle($OnText, $OffText, $Left, $Top, $Width, $Height, $BG_Color = $GUIThemeColor, $Font_Color = $FontThemeColor, $Font = "Segoe UI", $Fontsize = "11")
	Return _SCN_CreateToggle($OffText & "|@|" & $OnText, $Left, $Top, $Width, $Height, $BG_Color, $Font_Color, $Font, $Fontsize)
EndFunc   ;==>_SCN_CreateOnOffToggle
...
; Return values .: Handle to the toggle.
; ===============================================================================================================================
Func _SCN_CreateOnOffToggleEx($OnText, $OffText, $Left, $Top, $Width, $Height, $BG_Color = $GUIThemeColor, $Font_Color = $FontThemeColor, $Font = "Segoe UI", $Fontsize = "11")
	Return _SCN_CreateToggleEX($OffText & "|@|" & $OnText, $Left, $Top, $Width, $Height, $BG_Color, $Font_Color, $Font, $Fontsize)
EndFunc   ;==>_SCN_CreateOnOffToggleEx


; #FUNCTION# ====================================================================================================================
; Name ..........: _SCN_ToggleIsChecked
; Description ...: Checks if a toggle is checked
; Syntax ........: _SCN_ToggleIsChecked($Toggle)
; Parameters ....: $Toggle              - Handle of the toggle.
; Return values .: True / False
; ===============================================================================================================================
Func _SCN_ToggleIsChecked($Toggle)
	For $i = 0 To (UBound($iHoverReg) - 1) Step +1
		...
		EndIf
	Next
EndFunc   ;==>_SCN_ToggleIsChecked

; #FUNCTION# ====================================================================================================================
; Name ..........: _SCN_ToggleSwitch
...
; ===============================================================================================================================
Func _SCN_ToggleSwitch($Toggle)
	If _SCN_ToggleIsChecked($Toggle) Then
		_SCN_ToggleUnCheck($Toggle)
		Return False
	Else
		_SCN_ToggleCheck($Toggle)
		Return True
	EndIf
EndFunc   ;==>_SCN_ToggleSwitch

; #FUNCTION# ====================================================================================================================
; Name ..........: _SCN_ToggleUnCheck
; Description ...: Unchecks a toggle
; Syntax ........: _SCN_ToggleUnCheck($Toggle[, $NoAnimation = False])
...
Func _SCN_ToggleUnCheck($Toggle, $NoAnimation = False)
	For $i = 0 To (UBound($iHoverReg) - 1) Step +1
		...
		EndIf
	Next
EndFunc   ;==>_SCN_ToggleUnCheck


; #FUNCTION# ====================================================================================================================
; Name ..........: _SCN_ToggleCheck
; Description ...: Checks a toggle
; Syntax ........: _SCN_ToggleCheck($Toggle[, $NoAnimation = False])
; Parameters ....: $Toggle              - Handle to the toggle.
...
; Syntax ........: _SCN_CreateRadio($RadioGroup, $Text, $Left, $Top, $Width, $Height[, $BG_Color = $GUIThemeColor[,
...
; ===============================================================================================================================
Func _SCN_CreateRadio($RadioGroup, $Text, $Left, $Top, $Width, $Height, $BG_Color = $GUIThemeColor, $Font_Color = $FontThemeColor, $Font = "Segoe UI", $Fontsize = "11", $FontStyle = 0, $RadioCircleSize = 22, $ExStyle = False)
...
EndFunc   ;==>_SCN_CreateRadio

; #FUNCTION# ====================================================================================================================
; Name ..........: _SCN_CreateRadioEx
...
; Return values .: Handle to the radio.
; ===============================================================================================================================
Func _SCN_CreateRadioEx($RadioGroup, $Text, $Left, $Top, $Width, $Height, $BG_Color = $GUIThemeColor, $Font_Color = $FontThemeColor, $Font = "Segoe UI", $Fontsize = "11", $FontStyle = 0, $RadioCircleSize = 22)
	Return _SCN_CreateRadio($RadioGroup, $Text, $Left, $Top, $Width, $Height, $BG_Color, $Font_Color, $Font, $Fontsize, $FontStyle, $RadioCircleSize, True)
EndFunc   ;==>_SCN_CreateRadioEx

; #FUNCTION# ====================================================================================================================
; Name ..........: _SCN_RadioCheck
; Description ...: Checks the selected radio and unchecks all other radios in the same radiogroup.
; Syntax ........: _SCN_RadioCheck($RadioGroup, $Radio)
...
; Syntax ........: _SCN_RadioIsChecked($RadioGroup, $Radio)
; Parameters ....: $RadioGroup          - Radio group
;				   $Radio				- Handle to the radio
; Return values .: True / False
; ===============================================================================================================================
Func _SCN_RadioIsChecked($RadioGroup, $Radio)
	For $i = 0 To (UBound($iHoverReg) - 1) Step +1
		If $iHoverReg[$i][0] = $Radio Then
			If $iHoverReg[$i][4] = $RadioGroup Then
				If $iHoverReg[$i][2] Then
...
EndFunc   ;==>_SCN_CreateCheckbox

; #FUNCTION# ====================================================================================================================
; Name ..........: _SCN_CreateCheckboxEx
; Description ...: Creates a checkbox with the old black and white style.
; Syntax ........: _SCN_CreateCheckboxEx($Text, $Left, $Top, $Width, $Height[, $BG_Color = $GUIThemeColor[, $Font_Color = $FontThemeColor[,
...
; Return values .: Handle to the Checkbox
; ===============================================================================================================================
Func _SCN_CreateCheckboxEx($Text, $Left, $Top, $Width, $Height, $BG_Color = $GUIThemeColor, $Font_Color = $FontThemeColor, $Font = "Segoe UI", $Fontsize = "11", $FontStyle = 0)
	Return _SCN_CreateCheckbox($Text, $Left, $Top, $Width, $Height, $BG_Color, $Font_Color, $Font, $Fontsize, $FontStyle, 0)
EndFunc   ;==>_SCN_CreateCheckboxEx

; #FUNCTION# ====================================================================================================================
; Name ..........: _SCN_CreateCheckboxEx2
; Description ...: Creates a modern rounded checkbox
; Syntax ........: _SCN_CreateCheckboxEx2($Text, $Left, $Top, $Width, $Height[, $BG_Color = $GUIThemeColor[,
;                  $Font_Color = $FontThemeColor[, $Font = "Segoe UI"[, $FontSize = "11"[, $FontStyle = 0]]]]])
; Parameters ....: $Text                - Text.
;                  $Left                - Left pos.
;                  $Top                 - Top pos.
...
Func _SCN_CreateCheckboxEx2($Text, $Left, $Top, $Width, $Height, $BG_Color = $GUIThemeColor, $Font_Color = $FontThemeColor, $Font = "Segoe UI", $Fontsize = "11", $FontStyle = 0)
	...
	_cHvr_Register($Checkbox_Array[0], "_iHoverOff", "_iHoverOn", "", "", _iAddHover($Checkbox_Array))
	Return $Checkbox_Array[0]
EndFunc   ;==>_SCN_CreateCheckboxEx2

; #FUNCTION# ====================================================================================================================
; Name ..........: _SCN_CheckboxIsChecked
; Description ...: Checks if a metro checkbox is checked.
; Syntax ........: _SCN_CheckboxIsChecked($Checkbox)
; Parameters ....: $Checkbox            - Handle to the checkbox.
; Return values .: True / False
; ===============================================================================================================================
Func _SCN_CheckboxIsChecked($Checkbox)
	For $i = 0 To (UBound($iHoverReg) - 1) Step +1
		...
		EndIf
	Next
EndFunc   ;==>_SCN_CheckboxIsChecked

; #FUNCTION# ====================================================================================================================
; Name ..........: _SCN_CheckboxUnCheck
; Description ...: Unchecks a metro checkbox
; Syntax ........: _SCN_CheckboxUnCheck($Checkbox,)
; Parameters ....: $Checkbox            - Handle to the Checkbox.
; ===============================================================================================================================
Func _SCN_CheckboxUnCheck($Checkbox)
	For $i = 0 To (UBound($iHoverReg) - 1) Step +1
		If $iHoverReg[$i][0] = $Checkbox Then
			$iHoverReg[$i][2] = False
			$iHoverReg[$i][1] = True
		...
		EndIf
	Next
EndFunc   ;==>_SCN_CheckboxUnCheck

; #FUNCTION# ====================================================================================================================
; Name ..........: _SCN_CheckboxCheck
; Description ...: Checks a metro checkbox
; Syntax ........: _SCN_CheckboxCheck($Checkbox, $NoHoverEffect = False)
; Parameters ....: $Checkbox            - Handle to the Checkbox.
...
Func _SCN_CheckboxCheck($Checkbox, $NoHoverEffect = False)
	For $i = 0 To (UBound($iHoverReg) - 1) Step +1
		...
		EndIf
	Next
EndFunc   ;==>_SCN_CheckboxCheck

; #FUNCTION# ====================================================================================================================
; Name ..........: _SCN_CheckboxSwitch
...
; Syntax ........: _SCN_CheckboxSwitch($Checkbox)
; Parameters ....: $Checkbox            - Handle to the Checkbox.
; Returns ---------: True = Checkbox is checked, False = Checkbox is not checked.
; ===============================================================================================================================
Func _SCN_CheckboxSwitch($Checkbox)
	If _SCN_CheckboxIsChecked($Checkbox) Then
		_SCN_CheckboxUnCheck($Checkbox)
		Return False
	Else
		_SCN_CheckboxCheck($Checkbox)
		Return True
	EndIf
...
; Syntax ........: _SCN_MsgBox($Flag, $Title, $Text[, $mWidth = 600[, $FontSize = 14[, $ParentGUI = "", $Timeout = 0]]])
...
Func _SCN_MsgBox($Flag, $Title, $Text, $mWidth = 600, $Fontsize = 11, $ParentGUI = "", $Timeout = 0)
	...
	Switch $Flag
		...
		Case 1 ;OK / Cancel
			$Buttons_Count = 2
			$1stButtonText = "OK"
			$2ndButtonText = "Huỷ bỏ"
		Case 2 ;Abort / Retry / Ignore
			$Buttons_Count = 3
			$1stButtonText = "Huỷ bỏ"
			$2ndButtonText = "Thử lại"
			$3rdButtonText = "Lờ đi"
		Case 3 ;Yes / NO / Cancel
			$Buttons_Count = 3
			$1stButtonText = "Có"
			$2ndButtonText = "Không"
			$3rdButtonText = "Huỷ bỏ"
		Case 4 ;Yes / NO
			$Buttons_Count = 2
			$1stButtonText = "Có"
			$2ndButtonText = "Không"
		Case 5 ; Retry / Cancel
			$Buttons_Count = 2
			$1stButtonText = "Thử lại"
			$2ndButtonText = "Huỷ bỏ"
		Case 6 ; Cancel / Retry / Continue
			$Buttons_Count = 3
			$1stButtonText = "Huỷ bỏ"
			$2ndButtonText = "Thử lại"
			$3rdButtonText = "Tiếp tục"
		Case Else
			$Buttons_Count = 1
			$1stButtonText = "OK"
	EndSwitch
	...
	Local $1stButton = _SCN_CreateButton($1stButtonText, $1stButton_Left, ($mHeight / $msgbDPI) - 50, 100, 34, $ButtonBKColor, $ButtonTextColor)
	Local $2ndButton = _SCN_CreateButton($2ndButtonText, $2ndButton_Left, ($mHeight / $msgbDPI) - 50, 100, 34, $ButtonBKColor, $ButtonTextColor)
	If $Buttons_Count < 2 Then GUICtrlSetState($2ndButton, 32)
	Local $3rdButton = _SCN_CreateButton($3rdButtonText, $3rdButton_Left, ($mHeight / $msgbDPI) - 50, 100, 34, $ButtonBKColor, $ButtonTextColor)
...
; Syntax ........: _SCN_InputBox($Promt[, $Font_Size = 11[, $DefaultText = ""[, $PW = False[, $EnableEnterHotkey = True[,
;                  $ParentGUI = ""]]]]])
...
Func _SCN_InputBox($Promt, $Font_Size = 11, $DefaultText = "", $PW = False, $EnableEnterHotkey = True, $ParentGUI = "")
	...
	Else
		$Metro_Input_GUI = _SCN_CreateGUI(WinGetTitle($ParentGUI, "") & ".Input", 460, 170, -1, -1, False, $ParentGUI)
	EndIf
	_SCN_SetGUIOption($Metro_Input_GUI, True)
	...
	Local $cEnter = GUICtrlCreateDummy()
	Local $aAccelKeys[1][2] = [["{ENTER}", $cEnter]]
	Local $Button_Continue = _SCN_CreateButtonEx2("Continue", 110, 120, 100, 36, $ButtonBKColor, $ButtonTextColor, "Segoe UI")
	GUICtrlSetState($Button_Continue, 512)
	Local $Button_Cancel = _SCN_CreateButtonEx2("Cancel", 230, 120, 100, 36, $ButtonBKColor, $ButtonTextColor, "Segoe UI")
	...
	While 1
		...
		Switch $input_nMsg
			Case -3, $Button_Cancel
				_SCN_GUIDelete($Metro_Input_GUI)
				If $mOnEventMode Then Opt("GUIOnEventMode", 1) ;Reactivate oneventmode
				Return SetError(1, 0, "")
...
; Syntax ........: _SCN_CreateProgress($Left, $Top, $Width, $Height[, $EnableBorder = False[, $Backgroud_Color = $CB_Radio_Color[,
...
EndFunc   ;==>_SCN_CreateProgress

; #FUNCTION# ====================================================================================================================
; Name ..........: _SCN_SetProgress
; Description ...: Sets the progress in % of a progressbar.
; Syntax ........: _SCN_SetProgress(Byref $Progress, $Percent)
; Parameters ....: $Progress            - Array of the progressbar that has been returned by _SCN_CreateProgress function.
;                  $Percent             - A value from 0-100. (In %)
; ===============================================================================================================================
Func _SCN_SetProgress(ByRef $Progress, $Percent)
	;Set Colors
...
; #FUNCTION# ====================================================================================================================
; Name ..........: _SCN_AddHSeperator
; Description ...: Adds a horizontal seperator line to the GUI
; Syntax ........: _SCN_AddHSeperator($Left, $Top, $Width, $Size[, $Color = $GUIBorderColor])
...
Func _SCN_AddHSeperator($Left, $Top, $Width, $Size, $Color = $GUIBorderColor)
	...
	Return $Seperator
EndFunc   ;==>_SCN_AddHSeperator


; #FUNCTION# ====================================================================================================================
; Name ..........: _SCN_AddVSeperator
; Description ...: Adds a vertical seperator line to the GUI
; Syntax ........: _SCN_AddVSeperator($Left, $Top, $Height, $Size[, $Color = $GUIBorderColor])
; Parameters ....: $Left                - x Position.
;                  $Top                 - y Position
;                  $Height               - Height
;                  $Size                - Size of the line
...
Func _iMExit()
	For $i_HR = 0 To UBound($iGUI_LIST) - 1 Step +1
		_SCN_GUIDelete($iGUI_LIST[$i_HR][0])
	Next
